...
def distance_betw_contours(contourList, idx1, idx2):
    import math
    p1 = contourList[idx1].reshape(4,2)[3]
    p2 = contourList[idx2].reshape(4,2)[3]
...
def findRollNumber(contourList, grayImage):
    ...
    warped = cv2.adaptiveThreshold(warped, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2)
    #warped = warped[int(warped.shape[0]/100):, int(warped.shape[1]/14):]
    ...
    #warped = warped[int(warped.shape[0]/9):int(warped.shape[0]*0.995), int(warped.shape[1]/14):int(warped.shape[0]*0.997)]
